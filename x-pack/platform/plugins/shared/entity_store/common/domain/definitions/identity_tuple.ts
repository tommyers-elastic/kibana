...
/**
 * Fixture and test helper, not schema: builds the core `identityField` block for a tuple of
 * literal field paths, in the shape the inventory query generator serves (see
 ...
 *
 * Authored definitions declare `identityField` directly; this helper only saves fixtures from
 * spelling the block out. Throws when a field is not a literal path (expressions, wildcards and
 * whitespace are rejected) or when the list is empty or contains duplicates.
 */
